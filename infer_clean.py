...
# Safe guard in case it is omitted
...
for snr, ser in avg_ser.items():
    snr_lbl = f"{snr:>3}" if isinstance(snr, (int, float)) else snr
    print(f"  SNR={snr_lbl} dB -> SER={ser:.4f}")

# save JSON
json.dump(
    {"per_seed": seed_results,
     "avg_ser": {str(k): v for k, v in avg_ser.items()}},
